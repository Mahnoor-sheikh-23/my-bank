...
// all ingormation collected here 
    customerInfo(name:string,fname:string,age:number,gender:string,mobile:number){
...
class BankAccount{
    ...
    // class method
    debbit(pin:number,amount:number){
        let check = custom.id == pin
        if(!check){
            console.log(chalk.italic.rgb(300,0,70)("Your PIN CODE  is incorrect"))
        }
        else{
         let statement : string = "Sorry ! you have unsufficient balance "
            if(amount > 0 ){
    ...
    // class second method
    creedit(pin:number,amount:number){
        let check = custom.id == pin
